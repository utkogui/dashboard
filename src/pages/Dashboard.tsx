    ...
    <div style={{ padding: 24, background: '#f5f5f5', minHeight: '100vh' }}>
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        ...
        <div>
          ...
          {/* Profissionais - organizado em tabela */}
          <Card style={{ marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <Title level={4} style={{ margin: 0 }}>
            ...
            <Table
              dataSource={profissionais.slice().sort((a, b) => a.nome.localeCompare(b.nome))}
              rowKey="id"
              onRow={(record) => ({
                onClick: () => handleCardClick('profissional', record),
              ...
              columns={[
                {
                  ...
                  render: (text, record: any) => {
                    const ocupado = contratos.some(c => c.status === 'ativo' && c.profissionais.some(p => p.profissionalId === record.id))
                ...
                },
                {
                  ...
                  key: 'tipoContrato',
                  render: (text: string) => (
                    <Tag color={text === 'hora' ? 'blue' : 'green'}>
                      {text === 'hora' ? 'Por Hora' : 'Valor Fechado'}
                    </Tag>
                  ),
                },
                {
                  ...
                  render: (record: any) => (
                    <Text strong>
                      {record.tipoContrato === 'hora' 
                        ? `R$ ${(record.valorHora || 0).toLocaleString('pt-BR')}/h`
                        : `R$ ${(record.valorFechado || 0).toLocaleString('pt-BR')}`
                      }
                    </Text>
                  ),
                  sorter: (a: any, b: any) => {
